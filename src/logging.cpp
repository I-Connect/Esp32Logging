  ...
  public:
      ...
      : _bufsize(bufsize), _flush_period_ms(autoFlushPeriod)
    {
      ...
      xTaskCreate([](void* self) {
        ((LogQueue*)self)->run();
      }, "esp32m::log-queue", 4096, this, tskIDLE_PRIORITY, &_task);
      logQueue = this;
    }
    ~LogQueue()
    {
...
void Logger::makeMessageAndAppend(LogLevel level, const char* msg)
{
...
void Logger::structured_logf(LogLevel level,
                             const char* filename,
                             int linenumber,
                             const char* function,
                             const char* format, ...)
{
  ...
  va_list arg;
  va_start(arg, format);
  structured_logf(level, filename, linenumber, function, format, arg);
  va_end(arg);
}
...
void Logger::logf(LogLevel level, const char* format, va_list arg)
{
  ...
  vsnprintf(temp, len + 1, format, arg);
  makeMessageAndAppend(level, temp);
  if (len >= sizeof(buf)) {
    free(temp);
  }
}
    ...
    void hookImpl(char c)
    {
      ...
      if (b && bl)
      {
        if (c == '\n' || _serialBufPtr >= bl - 1)
        {
          ...
          auto level = detectLevel(mptr);
          xSemaphoreGive(_lock);
          Logging::system().makeMessageAndAppend(level, *mptr);
          xSemaphoreTake(_lock, portMAX_DELAY);
        }
        if (c != '\n' && c != '\r') {
          b[_serialBufPtr++] = c;
